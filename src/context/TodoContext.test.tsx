...
describe('TodoContext', () => {
  test('fetches and provides todos', async () => {
    ...
    await waitFor(() => {
      ...
      mockTodos.forEach((todo, index) => {
        expect(htmlElements[index]).toHaveTextContent(
          `${todo.id}${todo.text}${todo.isDone ? 'Done' : 'Not Done'}`,
        )
      })
    })
  })
  ...
  test('adds a new todo', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByTestId('todo-item')).toHaveLength(
        mockTodos.length + 1,
      )
  ...
  test('updates a todo status', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(`${mockTodos[0].text} - Done`),
      ).toBeInTheDocument()
    })
  })
})
